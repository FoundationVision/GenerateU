...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if not use_focal_loss:
                ...
                loss_fct = FocalLoss(gamma=2.0, ignore_index=-100, reduction=reduction)
                m = nn.Softmax(dim=-1)
                loss = loss_fct(m(lm_logits.view(-1, lm_logits.size(-1))), labels.view(-1))
    ...
    def get_contrastive_label(self, object_descriptions, output_tokens_id, output_tokens_attention_mask, box_features, negative_description_mask):
        text_tokens = []
        text_lengths = []
        need_predict = []
        contrastive_label = torch.zeros((box_features.size(0)), dtype=torch.int64).to(box_features.device)
        exsit_label = []
        text_tokens_for_enc = []
        text_tokens_mask = []
        description_to_exsit_label = [] #torch.zeros((len(object_descriptions)), dtype=torch.int64).to(box_features.device)

        for ii, description in enumerate(object_descriptions):
            if description not in exsit_label:
                exsit_label.append(description)
                contrastive_label[ii] = len(exsit_label)-1
                text_tokens_for_enc.append(output_tokens_id[ii:ii+1])
                text_tokens_mask.append(output_tokens_attention_mask[ii:ii+1])
                description_to_exsit_label.append(ii)
            else:
                contrastive_label[ii] = exsit_label.index(description)
        description_to_exsit_label = torch.tensor(description_to_exsit_label, dtype=int)
        negative_description_mask = negative_description_mask[:,description_to_exsit_label]
                
        #convert contrastive_label to one-hot
        contrastive_label = torch.zeros(len(contrastive_label), contrastive_label.max()+1).to(box_features.device).scatter_(1, contrastive_label.unsqueeze(1), 1.)
        pos_neg_description_mask = negative_description_mask + contrastive_label
        text_tokens_for_enc = torch.cat(text_tokens_for_enc, dim=0).to(box_features.device)
        text_tokens_mask = torch.cat(text_tokens_mask, dim=0).to(box_features.device)

        # contrastive_label = contrastive_label.to(dtype=torch.int64)
        data = {'text_tokens_for_enc': text_tokens_for_enc,
                'text_tokens_mask': text_tokens_mask,
                'contrastive_label': contrastive_label,
                'pos_neg_description_mask': pos_neg_description_mask}

        return data

    def img_text_contrastive(
        self,
        batch
    ): 
        inputs_embeds = batch['inputs_embeds']
        text_tokens_for_enc = batch['text_tokens_for_enc'] 
        contrastive_label = batch['contrastive_label'] 
        pos_neg_description_mask = batch['pos_neg_description_mask'] 
        object_encoder_atts = batch['encoder_atts']
        text_tokens_mask = batch['text_tokens_mask']
        use_all_negative = batch['use_all_negative']
        

        object_encoder_outputs = self.encoder(
                        input_ids=None, #input_ids,
                        attention_mask=object_encoder_atts,
                        inputs_embeds=inputs_embeds,
                        head_mask=None, #head_mask,
                        output_attentions=None, #output_attentions,
                        output_hidden_states=None, #output_hidden_states,
                        return_dict=True, #return_dict,
                    )
        object_hidden_states = object_encoder_outputs[0][:,0,:]
        text_encoder_outputs = self.encoder(
                        input_ids=text_tokens_for_enc, #input_ids,
                        attention_mask=text_tokens_mask,
        ...
        text_hidden_states = text_encoder_outputs[0][:,0,:]
    
        sims_q2t = torch.matmul(object_hidden_states, text_hidden_states.transpose(0, 1))

        pos_neg_description_mask = torch.tensor(pos_neg_description_mask, dtype=bool)

        if use_all_negative:
            targets = contrastive_label
            loss_itc = F.cross_entropy(sims_q2t/ self.temp , targets, label_smoothing=0.1)

        else:  #use_diff_img_negative: 
            loss_itc = torch.zeros(1).to(sims_q2t.device)
            for sim_q2t, label, mask in zip(sims_q2t, contrastive_label, pos_neg_description_mask):
                assert sim_q2t.size() == label.size()
                assert mask.size() == label.size()
                loss_itc += F.cross_entropy(sim_q2t.masked_select(mask).unsqueeze(0)/ self.temp , label.masked_select(mask).unsqueeze(0), label_smoothing=0.1)          
            bs = sims_q2t.size(0)
            loss_itc = loss_itc/bs

        return {'img_text_contrastive_loss': loss_itc}
