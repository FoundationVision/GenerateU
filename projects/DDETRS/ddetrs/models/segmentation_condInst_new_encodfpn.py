...
class DETRsegmVLUni(nn.Module):
    ...
    def forward(self, samples, gt_targets, criterion, train=False, clip_object_descriptions_features=[], dataset_source=0, ann_type='box'):
        ...
        for lvl in range(enc_lay_num):
            ...
            outputs_class = self.detr.class_embed[lvl](hs[lvl], clip_object_descriptions_features)
        ...
        if ann_type == 'image':
            ...
            topk_values, topk_indices =  torch.topk(outputs_classes[-1][:,:,0], k=topk_p, dim=1) # outputs_classes[-1]:[BS, Query, class_num]  [1, 300, 1]
            object_features = torch.zeros((hs.size(1), topk_p, hs.size(-1))).to(hs.device) # [BS,topk_p,dim] #[1, 10, 256]
            object_features_att_mask = torch.zeros(object_features.size()[:-1], dtype=torch.long).to(hs.device) # [BS,topk_p]
            ...
            text_decoder_loss = self.detr.class_generate(object_features, object_descriptions, object_features_att_mask)
            ...
            for i, (indice, x_gt) in enumerate(zip(indices, gt_targets)):
                pred_i, tgt_j = indice
                object_descriptions += [x_gt['gt_object_descriptions'][x_j] for x_j in tgt_j]
                object_features.append(hs[-1][i][pred_i])
            object_features = torch.cat(object_features, 0).unsqueeze(1)
            object_features_att_mask = torch.ones(object_features.size()[:-1], dtype=torch.long).to(hs.device)
            text_decoder_loss = self.detr.class_generate(object_features, object_descriptions, object_features_att_mask)
        ...
        if self.detr.aux_loss:
            if self.use_iou_branch:
                outputs['aux_outputs'] = self._set_aux_loss_with_iou(outputs_classes, outputs_coord, outputs_mask, outputs_iou)
            else:
                outputs['aux_outputs'] = self._set_aux_loss(outputs_classes, outputs_coord, outputs_mask)
