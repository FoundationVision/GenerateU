...
# V2 supports (1) whether to use LayerNorm (2) different gamma for different tasks
class BiAttentionBlockForCheckpointV2(nn.Module):
    def __init__(self, v_dim, l_dim, embed_dim, num_heads, dropout=0.1,
                 drop_path=.0, init_values=1e-4, cfg=None):
        """
        Inputs:
            ...
            dropout - Amount of dropout to apply in the feed-forward network
        """
        super(BiAttentionBlockForCheckpointV2, self).__init__()

        # pre layer norm
        self.use_layernorm = cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_LAYERNORM
        ...
        # add layer scale for training stability
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.decouple_gamma = cfg.MODEL.DYHEAD.FUSE_CONFIG.DECOUPLE_GAMMA
        if self.decouple_gamma:
            self.gamma_v = nn.ParameterDict({
                "detection": nn.Parameter(init_values * torch.ones((v_dim)), requires_grad=True),
                "grounding": nn.Parameter(init_values * torch.ones((v_dim)), requires_grad=True),
                "sot": nn.Parameter(init_values * torch.ones((v_dim)), requires_grad=True)
                })
            self.gamma_l = nn.ParameterDict({
                "detection": nn.Parameter(init_values * torch.ones((l_dim)), requires_grad=True),
                "grounding": nn.Parameter(init_values * torch.ones((l_dim)), requires_grad=True),
                "sot": nn.Parameter(init_values * torch.ones((l_dim)), requires_grad=True)
                })
        else:
            self.gamma_v = nn.Parameter(init_values * torch.ones((v_dim)), requires_grad=True)
            self.gamma_l = nn.Parameter(init_values * torch.ones((l_dim)), requires_grad=True)
    ...
    def forward(self, v, l, attention_mask_l=None, task=None):
        ...
        if self.decouple_gamma:
            ...
            v = v + self.drop_path(self.gamma_v[task] * delta_v)
            l = l + self.drop_path(self.gamma_l[task] * delta_l)
            dummy_v, dummy_l = 0, 0
            # avoid unused parameters
            for _, x in self.gamma_v.items():
                dummy_v += 0.0 * torch.sum(x)
            for _, x in self.gamma_l.items():
                dummy_l += 0.0 * torch.sum(x)
            v = v + dummy_v
            l = l + dummy_l
        else:
            v = v + self.drop_path(self.gamma_v * delta_v)
