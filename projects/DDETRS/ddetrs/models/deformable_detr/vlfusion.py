import torch
from .fuse_helper import BiAttentionBlockForCheckpoint, BiAttentionBlockForCheckpointV2
...
class VLFuse(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.DYHEAD.FUSE_CONFIG.FUSE_TYPE == "V1":
        ...
        elif cfg.MODEL.DYHEAD.FUSE_CONFIG.FUSE_TYPE == "V2":
            # bi-direction (text->image, image->text)
            self.b_attn = BiAttentionBlockForCheckpointV2(v_dim=self.img_dim, # 256
                        l_dim=self.lang_dim, # 768
                        embed_dim=self.embed_dim, # 2048
                        num_heads=self.n_head, # 8
                        dropout=cfg.MODEL.DDETRS.DROPOUT, # 0.0
                        drop_path=.0,
                        init_values=cfg.MODEL.DYHEAD.FUSE_CONFIG.GAMMA_INIT_VALUE,
                        cfg=cfg
                        )
        else:
