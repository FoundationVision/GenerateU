...
class Attention(nn.Module):
    def __init__(
        ...
        self.rel_pos_h = nn.Embedding(rel_sp_dim, head_dim, _weight=torch.zeros(rel_sp_dim, head_dim, requires_grad=True))
        self.rel_pos_w = nn.Embedding(rel_sp_dim, head_dim, _weight=torch.zeros(rel_sp_dim, head_dim, requires_grad=True))
    ...
    def forward(self, x, H, W, rel_pos_bias=None):
        ...
        attn = calc_rel_pos_spatial(attn, q, self.window_size, self.window_size, self.rel_pos_h.weight, self.rel_pos_w.weight)
...
class ViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12,
                 ...
                 pretrained=None, pos_emb_form="learnable"):
        super().__init__()
        assert pos_emb_form in ["learnable", "sine"]
        print("pos_emb_form of the ViT backbone is %s" %pos_emb_form)
        interval = depth // 4 # as mentioned in ViTDet, there are 4 propagation blocks
        ...
        if use_abs_pos_emb:
            if pos_emb_form == "learnable":
                # self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))
                self.pos_embed = nn.Embedding(num_patches, embed_dim, _weight=torch.zeros(num_patches, embed_dim, requires_grad=True))
            elif pos_emb_form == "sine":
                pos_embed = torch.from_numpy(get_2d_sincos_pos_embed(embed_dim, int(self.patch_embed.num_patches**.5), cls_token=False)).float()
                self.pos_embed = nn.Embedding(num_patches, embed_dim, _weight=pos_embed) # for fixed sin-cos encoding
        ...
        if self.pos_embed is not None and pos_emb_form == "learnable":
        ...
        #     nn.ConvTranspose2d(embed_dim, embed_dim, kernel_size=2, stride=2),
        # )
        self.fpn1 = nn.Identity() # In our method, we don't use backbone feature with stride 4
    ...
    def forward(self, x):
        ...
        rets = {"res{}".format(u + 2): v for (u,v) in enumerate(x)}
        return rets
...
class D2ViT(ViT, Backbone):
    def __init__(self, cfg, input_shape):
        ...
        pos_emb_form = cfg.MODEL.VIT.POS_EMB_FORM
        mlp_ratio=4
        qkv_bias=True
        qk_scale=None
        drop_rate=0.
        ...
        super().__init__(img_size=img_size, embed_dim=embed_dim, depth=depth, 
        ...
        use_abs_pos_emb=use_abs_pos_emb, pos_emb_form=pos_emb_form)
        ...
        self._out_feature_channels = {
            "res3": self.num_features,
            "res4": self.num_features,
            "res5": self.num_features,
        }
